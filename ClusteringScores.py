...
def davisbouldin(k_list, k_centers):
    ...
    """
    big_ss = np.zeros([len(k_list)], dtype=np.float64)
    d_eucs = np.zeros([len(k_list), len(k_list)], dtype=np.float64)
    db = 0    
    ...
    for k in range(len(k_list)):
        for l in range(0, len(k_list)):
            d_eucs[k, l] = np.linalg.norm(k_centers[k]-k_centers[l])

    for k in range(len(k_list)):
        values = np.zeros([len(k_list)-1], dtype=np.float64)
        for l in range(0, k):
            values[l] = (big_ss[k] + big_ss[l])/d_eucs[k, l]
        for l in range(k+1, len(k_list)):
